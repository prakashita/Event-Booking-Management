...
def serialize_publication(item: Publication) -> PublicationResponse:
    return PublicationResponse(
        ...
        source_type=getattr(item, "source_type", None) or item.pub_type,
        citation_format=getattr(item, "citation_format", None),
        ...
        author_first_name=getattr(item, "author_first_name", None),
        author_last_name=getattr(item, "author_last_name", None),
        publication_date=getattr(item, "publication_date", None),
        issued_date=getattr(item, "issued_date", None),
        accessed_date=getattr(item, "accessed_date", None),
        composed_date=getattr(item, "composed_date", None),
        submitted_date=getattr(item, "submitted_date", None),
        content=getattr(item, "content", None),
        contributors=getattr(item, "contributors", None),
        container_title=getattr(item, "container_title", None),
        collection_title=getattr(item, "collection_title", None),
        note=getattr(item, "note", None),
        source=getattr(item, "source", None),
        url=getattr(item, "url", None),
        pdf_url=getattr(item, "pdf_url", None),
        article_title=getattr(item, "article_title", None),
        journal_name=getattr(item, "journal_name", None),
        volume=getattr(item, "volume", None),
        issue=getattr(item, "issue", None),
        pages=getattr(item, "pages", None),
        doi=getattr(item, "doi", None),
        year=getattr(item, "year", None),
        book_title=getattr(item, "book_title", None),
        ...
        report_title=getattr(item, "report_title", None),
        creator=getattr(item, "creator", None),
        video_title=getattr(item, "video_title", None),
        platform=getattr(item, "platform", None),
        newspaper_name=getattr(item, "newspaper_name", None),
        website_name=getattr(item, "website_name", None),
        page_title=getattr(item, "page_title", None),
    )
